...
public abstract class Comand {
    ...
    public static void removeAllByWeaponType(LinkedList<HumanBeing> collection, String str) {
        try {
            String str_type = trimString(str, "removeAllByWeaponType");
            WeaponType type;
            int index = -1;
            if (equals("", str)) {
                throw new NullException("weapon type");
            }
            type = getWeapon(str_type);
            for (int i = 0; i < collection.size(); i++) {
    ...
    public static void filterByMood(LinkedList<HumanBeing> collection, String str) {
        try {
            String str_type = trimString(str, "filterByMood");
            Mood type;
            int index = -1;
            if (equals("", str)) {
                throw new NullException("mood");
            }
            type = getMood(str_type);
            for (int i = 0; i < collection.size(); i++) {
    ...
    public static void save(LinkedList<HumanBeing> collection, String path) throws FileNotFoundException {
    ...
    public static void checkFile(File file, Boolean check){
        try {
            if (!file.exists()) {
                throw new FileNotFoundException();
            }
            if (!file.canRead() && check) {
                throw new AccessRightsException("read");
            }
            if (!file.canWrite()&& !check){
                throw new AccessRightsException("write");
            }
    ...
    public static void read(LinkedList<HumanBeing> collection, File file) {
        String str;
        int i = id + 1;
        try {
            checkFile(file, true);
